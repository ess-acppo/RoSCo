...
curl -s "https://${ROSCO_HOST}/service.php/find/ca_objects?authToken=${ROSCO_AUTH_TOKEN}&q=*" | jq '.'
echo
...
test_object_id_json=`curl -s "https://${ROSCO_HOST}/service.php/item/ca_objects/id/${test_object_id}?authToken=${ROSCO_AUTH_TOKEN}&pretty=1&format=edit"`
